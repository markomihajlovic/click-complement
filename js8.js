			...
			function kartyaFeltolt() {
				for(var j=0; j<db/2; j++) {
					...
					kartyakTomb2.push(j+db/2);	//h két "azonos" kártyára kattintva más hangszeren szólaljon meg
												//(a tárolt elemhez tartozó <audio> szól)
					
					kartyakTomb3.push(-1);	//csak h benne is legyen 2x db/2 elem
					kartyakTomb3.push(-1);	//elõször 0-t tettem be, ez baj volt
					
					//var hangszer = (j%2==0) ? 'viola/' : 'piano/';
					var strAud = '<audio id=audio' + j + '>';
					strAud += '<source src=viola/' + j + '.mp3 type="audio/mp3">';
					strAud += 'Your browser does not support HTML5 audio.';
					strAud += '</audio>'
					
					strAud += '<audio id=audio' + (j+db/2) + '>';
                ...
                if(kattintasokSzama == db)
					...
					if(mostani != elozo) {
							...
							if(felforditottDb==2) {
									...
									felforditottDb=0;
									if( Math.abs(kartyakTomb3[mostani] - kartyakTomb3[elozo]) == db/2)
									{
